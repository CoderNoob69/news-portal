...
export default function NewsUpload() {
    ...
    return (
        <Box sx={{ width: '100%', maxWidth: 700, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Card elevation={8} sx={{ borderRadius: 2, width: '100%', p: { xs: 3, sm: 6 }, boxShadow: 8, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                <CardContent sx={{ p: 0 }}>
                    ...
                    <Box component="form" onSubmit={e => { e.preventDefault(); uploadAll(); }} sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                        <TextField
                            label="Title"
                            value={title}
                        ...
                        />
                        <Box sx={{ mb: 1 }}>
                            <Typography variant="subtitle1" color="primary.dark" sx={{ mb: 1, fontWeight: 600 }}>
                                Attachments
                            </Typography>
                            <Box
                                {...getRootProps()}
                        ...
                        {rows.map(r => (
                            <Card key={r.file.name} sx={{ mb: 2, background: '#f8fbff', borderRadius: 2, boxShadow: 1 }}>
                                <CardContent sx={{ p: 2 }}>
                                    <Typography variant="subtitle2" fontWeight={700}>{r.file.name}</Typography>
                                    <Typography variant="caption" color="text.secondary">{r.size}</Typography>
                                    <Box sx={{ mt: 1, mb: 1 }}>
                                        <LinearProgress variant="determinate" value={r.pct} sx={{ height: 8, borderRadius: 4, background: '#eee' }} />
                                    </Box>
